...
if __name__ == "__main__":
    ...
    g_detector = get_detector(args)

    #det_loader = DetectionLoader(input_source, g_detector, cfg, args, batchSize=args.detbatch, mode=mode, queueSize=args.qsize)
    #det_worker = det_loader.start()

    virat_folder = '/content/TinyVIRAT_V2/videos/'
    virat_files = [(f, os.path.join(dp, f)) for dp, dn, fn in os.walk(virat_folder) for f in fn]
    ...
    try:
        for i, (file_name, file_path) in enumerate(virat_files):
            ...
            target_file = f'/content/drive/MyDrive/tinypose/{file_rel_path}'
            ...
            det_loader.cont(input_source)

            # Init data writer
            queueSize = 2 if mode == 'webcam' else args.qsize
            args.outputpath = target_file
